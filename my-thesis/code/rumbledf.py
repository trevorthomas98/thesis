...
chunk_size = 10000
df = []
for chunk in pd.read_csv(rumblecsv_file_path, chunksize=chunk_size):
    df.append(chunk)
rumbledf = pd.concat(df, ignore_index=True)
#youtubedf = pd.read_csv(youtubecsv_file_path)
print("rumble df.head is")
print(rumbledf.head)

num_columns = rumbledf.shape[1]
...
rumbledf.rename(columns={rumbledf.columns[40006]: videoid}, inplace=True)

crawljob = 'crawl_identifier'  # Specify the new column name

# Rename the 40006th column
rumbledf.rename(columns={rumbledf.columns[40005]: crawljob}, inplace=True)

numpackets = 'num_of_packets'  # Specify the new column name

# Rename the 40005th column
rumbledf.rename(columns={rumbledf.columns[40004]: numpackets}, inplace=True)

genre = 'genre'  # Specify the new column name

# Rename the 40004th column
rumbledf.rename(columns={rumbledf.columns[40003]: genre}, inplace=True)


platform = 'platform'  # Specify the new column name

# Rename the 40003th column
rumbledf.rename(columns={rumbledf.columns[40002]: platform}, inplace=True)


entryrelay = 'entry_ip'  # Specify the new column name

# Rename the 40002th column
rumbledf.rename(columns={rumbledf.columns[40001]: entryrelay}, inplace=True)

region = 'region'  # Specify the new column name

# Rename the 40001th column
rumbledf.rename(columns={rumbledf.columns[40000]: region}, inplace=True)

# Print the DataFrame to see the results
print(rumbledf)


output_file_path = '/home/trevor.thomas/rumbledf.pkl' 

# Save the DataFrame to a pickle file
rumbledf.to_pickle(output_file_path)
...
def main():
    ...
    learning_NoDef.learning(rumbledf)
